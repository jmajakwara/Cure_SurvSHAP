...
monte_carlo_cure <- function(B = 1000, n = 1000, scenario = "high_cure", seed = 2000) {
  ...
  # Track cure and censoring rates ===
  rates_summary <- data.frame(replication = 1:B,
                              cure_rate = NA_real_,
  ...
  true_values <- switch(scenario,
    high_cure = list(X1B = -1.5, X1C = -1.6, X1D = -2.45,`meanlog(X2)` = -5.65,
  ...
  for(b in 1:B) {
    ...
    dat <- generate_logn_cure_data(n = n, seed = b + 10000, scenario = scenario)
      ...
      # Record empirical rates for this replication
    rates_summary$cure_rate[b] <- mean(dat$cure)          # proportion cured
    rates_summary$censoring_rate[b] <- mean(dat$status == 0)
  ...
  metrics <- sim_results %>%
    ...
    # ====================== RATES SUMMARY ======================
  avg_rates <- rates_summary %>%
    summarise(
      mean_cure_rate = mean(cure_rate, na.rm = TRUE),
      mean_censoring_rate = mean(censoring_rate, na.rm = TRUE),
      sd_cure = sd(cure_rate, na.rm = TRUE),
      sd_censoring = sd(censoring_rate, na.rm = TRUE)
    )
