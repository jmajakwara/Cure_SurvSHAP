...
data_split <- initial_split(loans_cc, prop = 0.75, strata = status)     
...
n=20
#imp <- mice(loans_train_all, m=n,seed=2025,method=meth,predictorMatrix=quickpred(loans_train_all, mincor=0.1),printFlag=FALSE,maxit=40,ridge=0.01)
...
pdf(file="lnorm_imputations_plot_cured_pmm.pdf")
plot(imp)
dev.off()	


pdf(file="lnorm_densityplot_cured_pmm.pdf")
densityplot(imp)		
dev.off()
...
log_cure <- with(imp, flexsurvcure::flexsurvcure(Surv(time,status) ~ CreditScoreRange, anc = list(meanlog = ~  DebtToIncomeRatio +  BorrowerRate + OpenCreditLines + 
...
broom::tidy(pool(log_cure))
summary(pool(log_cure))
glance(pool(log_cure))
