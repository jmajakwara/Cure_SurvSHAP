...
generate_logn_cure_data <- function(n = 5000, seed = NULL) {
  ...
  X1 <- sample(c("A", "B", "C", "D"),
                        n, replace = TRUE,
                        prob = c(0.04, 0.2, 0.62, 0.14))
  
  X2 <- pmin(pmax(rnorm(n, 0.18, 0.05), 0.05), 0.35) # BorrowerRate
  X3 <- pmin(rlnorm(n, meanlog = log(0.15), sdlog = 0.85), 1.0)  # DebtToIncomeRatio
  X4 <- rpois(n, lambda = 8) # OpenCreditLines
  X5 <- runif(n, 0, 1)   # BankcardUtilization
  X6 <- rpois(n, lambda = 1.2)  # InquiriesLast6m
  X7 <- rbinom(n, 1, 0.55)  # IsHomeowner
  #X8 <- sample(c("1","2","3"), n, replace = TRUE, prob = c(0.13, 0.68, 0.19))  # Term
  ...
  # Incidence Submodel (log-odds of being SUSCEPTIBLE)
  b_true <- c(0.4, 0.55, 0.6, 0.8) # c(0.9, -0.45, -0.5, -0.7)   
  ...
  # Latency Submodel - Lognormal AFT
  g_true <- c(-5.08, -0.55, 0.03, 0.4, -0.06, -0.07)
  sigma  <- 0.77
  mu0 <- 7.5
